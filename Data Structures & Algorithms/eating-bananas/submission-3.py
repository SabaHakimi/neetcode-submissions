class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        while i < 50:
            ...
            print("testing ", bph, "from ", lower_bound, "and", upper_bound)
            hours = hoursUntilCompletion(bph, piles)
            print(bph, "takes", hours, "hours to complete")
            
            if hours > h:
                # search higher bph
                lower_bound = bph + 1
                print("new lower bound is", lower_bound)
            elif hours <= h:
                print(bph, "could be valid")
                if bph == 1 or hoursUntilCompletion(bph - 1, piles) > h:
                    return bph
                else:
                    # can optimize for lower bph
                    upper_bound = bph - 1
                    print("new upper bound is", upper_bound)
            i += 1
